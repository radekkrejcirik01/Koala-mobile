        ...
        >
          ...
          >
            {posting ? (
              ...
              <Text style={FriendsModalScreenStyle.sendButtonText}>Send</Text>
            )}
          </TouchableOpacity>
        </KeyboardAvoidingView>
      </View>
          ...
          <View style={FriendsModalScreenStyle.friendInvitesView}>
            {invites?.map((value: InviteInterface) => (
              <InviteItem
                ...
                item={value}
                posting={posting}
                onAcceptInvite={() => acceptInvite(value.username)}
            ...
            >
              {!!friends?.length && friends[index] ? (
                <FriendItem
                  ...
                  onPress={() =>
                    onFriendPress(friends[index]?.id, friends[index]?.name)
                  }
                  onLongPress={() =>
                    onFriendLongPress(friends[index]?.id, friends[index]?.name)
                  }
                  size={55}
                />
              ) : (
                <AddFriendButton size={55} onPress={() => setAdding(true)} />
              )}
            </View>
          ))}
        </View>
